...
public sealed partial class Ps
{
    ...
    public Result<PsOutput, Exception> Output()
    {
        ...
        var ec = child.Wait();
        if (ec.IsError)
            return PsResult.Err(ec.UnwrapError());

        var output = new PsOutput(this.FileName, ec.Unwrap(), stdOut, stdError, child.StartTime, child.ExitTime);
        return PsResult.Ok(output);
    }

    public async Task<Result<PsOutput, Exception>> OutputAsync(CancellationToken cancellationToken = default)
    {
        var stdOut = new List<string>();
        var stdError = new List<string>();
        this.StartInfo.Capture(stdOut);
