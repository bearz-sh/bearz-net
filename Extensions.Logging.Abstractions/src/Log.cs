...
public static class Log
{
    ...
    private static Lazy<ILogger> s_rootLogger = new(() => s_factory.CreateLogger(RootCategory ?? "ROOT"));

    public static string RootCategory { get; set; } = "ROOT";

    public static ILogger Root => s_rootLogger.Value;

    public static ILoggerFactory Factory
    {
        internal get => s_factory;
        set
        {
            if (s_rootLogger.IsValueCreated)
            {
                throw new InvalidOperationException(
                    "LoggerFactory cannot be changed after the first logger has been created.");
            }

            s_factory = value;
            s_rootLogger = new(() => s_factory.CreateLogger(RootCategory ?? "ROOT"));
        }
    }
