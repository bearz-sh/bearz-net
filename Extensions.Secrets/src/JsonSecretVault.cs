...
public sealed partial class JsonSecretVault : SecretVault, IDisposable
{
    ...
    public override string? GetSecretValue(string name)
    {
            ...
            && secret is not null)
        {
            if (secret.Value is { Length: > 0 })
            {
                ...
                return this.Options.Encoding.GetString(decrypted);
            }
    ...
    public override ISecretRecord? GetSecret(string name)
    {
            ...
            && secret is not null)
        {
            ...
            if (copy.Value is { Length: > 0 })
            {
                ...
                copy.Value = this.Options.Encoding.GetString(decrypted);
            }
    ...
    public void SetSecretValues(IEnumerable<KeyValuePair<string, string>> secrets)
    {
        ...
        foreach (var secret in secrets)
        {
            ...
            if (value.Length > 0)
            {
                var encrypted = this.cipher.Encrypt(this.Options.Encoding.GetBytes(value));
    ...
    private void Save()
    {
        ...
        try
        {
            ...
            var json = JsonSerializer.Serialize(vault, typeof(Vault), VaultContext.Default);
    ...
    private void Load(bool force = false)
    {
        ...
        try
        {
            ...
            if (JsonSerializer.Deserialize(json, typeof(Vault),  VaultContext.Default) is Vault vault)
            {
    ...
    internal partial class VaultContext : JsonSerializerContext
    {
    }
